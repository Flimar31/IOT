...
void reconnectMQTT() {
  while (!mqttClient.connected()) {
    ...
    String clientId = "ESP32_Projet1_";
...
void loop() {
  ...
  if (digitalRead(pinJaune)) {
    ...
    mqttClient.publish(mqtt_topic, "JAUNE");
    logger.info("Message MQTT envoyé : JAUNE");
    delay(250);
  }

  if (digitalRead(pinNoir)) {
    logger.logColor(ANSI_WHITE, "Bouton détecté : NOIR");
    mqttClient.publish(mqtt_topic, "NOIR");
    logger.info("Message MQTT envoyé : NOIR");
    delay(250);
  }

  if (digitalRead(pinRouge)) {
    logger.logColor(ANSI_RED, "Bouton détecté : ROUGE");
    mqttClient.publish(mqtt_topic, "ROUGE");
    logger.info("Message MQTT envoyé : ROUGE");
    delay(250);
  }

  if (digitalRead(pinBleu)) {
    logger.logColor(ANSI_BLUE, "Bouton détecté : BLEU");
    mqttClient.publish(mqtt_topic, "BLEU");
    logger.info("Message MQTT envoyé : BLEU");
    delay(250);
  }

  if (digitalRead(pinVert)) {
    logger.logColor(ANSI_GREEN, "Bouton détecté : VERT");
    mqttClient.publish(mqtt_topic, "VERT");
    logger.info("Message MQTT envoyé : VERT");
    delay(250);
  }
